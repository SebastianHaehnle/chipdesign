...
## PRIMARY PROPERTIES
   ...
   # s-parameter matrix
	...
	def f(self):
		...
		return self._frequency #self.frequency.f

	@f.setter
	def f(self,f):
#		tmpUnit = self.frequency.unit
		self._frequency  = f #Frequency(f[0],f[-1],len(f),'hz')
...
## CLASS METHODS
	...
	def read_touchstone(self, filename):
		...
		'''
		self.touchstoneFile = touchstone.touchstone(filename)
		if self.touchstoneFile.get_format().split()[1] != 's':
			raise NotImplementedError('only s-parameters supported for now.')

		self.f, self.s = self.touchstoneFile.get_sparameter_arrays() # note: freq in Hz
		self.z0 = float(self.touchstoneFile.resistance)
#		self.frequency.unit = self.touchstoneFile.frequency_unit # for formatting plots
		self.ff = self.touchstoneFile.ff
		self.feps = self.touchstoneFile.feps
		self.fz0 = self.touchstoneFile.fz0
	...
	def plot_z(self, m = None, ax = None, show_legend = True, *args, **kwargs):
		'''
		plots the port impedances indices m, n
		...
		for m in M:
			...
			# plot the desired attribute vs frequency
			ax.plot(self.ff,  self.fz0[:,m].real, *args,**kwargs)

		# Label Axis
		ax.set_xlabel('Frequency ['+ self.frequency.unit +']')
		ax.set_ylabel('Z0 [Ohm]')
		ax.axis('tight')
		ax.set_ylim([npy.floor(npy.min([self.fz0[:,m].real for m in M])), npy.ceil(npy.max([self.fz0[:,m].real for m in M]))])
