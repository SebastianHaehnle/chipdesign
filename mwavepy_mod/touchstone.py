...
class touchstone():
    ...
    def __init__(self,filename):
        ...
        ## simulated z0 (fz0) and epseff (feps) data for individual freqs given in ff for each port
        self.ff = None
        self.feps = None
        self.fz0 = None
    ...
    def load_file(self, filename):
        ...
        while (1):
            ...
            #Regular expression for scientific float (+-1.0e+-01)
            regex_float = r'[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?'
            # write frequency specific epseff and z0
            if 'Z0' in line:
                # Just append correct values, reshape array later afterwards
                start_i = line.index('F')+2
                self.ff.append(float(re.search(regex_float, line[start_i:]).group()))
                start_i = line.index('Eeff')+5
                fepstemp = float(re.search(regex_float, line[start_i:]).group())
                start_i = start_i + len(str(fepstemp)) + 1
                fepstemp += float(re.search(regex_float, line[start_i:]).group()) * 1j
                self.feps.append(fepstemp)
                start_i = line.index('Z0')+3
                z0temp = float(re.search(regex_float, line[start_i:]).group())
                start_i = start_i + len(str(z0temp)) + 1
                z0temp += float(re.search(regex_float, line[start_i:]).group()) * 1j
                self.fz0.append(z0temp)
                continue
        ...
        # Reshape frequency specific epseff, z0
        self.feps = numpy.reshape(numpy.array(self.feps), (-1, self.rank))
        self.fz0 = numpy.reshape(numpy.array(self.fz0), (-1, self.rank))
        self.ff = numpy.array(list(set(self.ff)))
        self.ff.sort()
        ...
        # Get rid of strange intermediate points which might fuck up frequency spacing
#        df = self.sparameters[1,0] - self.sparameters[0,0]
#        good_indices = numpy.where(numpy.isclose((self.sparameters[:,0]/df).astype(int),self.sparameters[:,0]/df, atol = 1e-12, rtol = 1e-10))
#        self.sparameters = self.sparameters[:][good_indices]
...
if __name__ == "__main__":
    ...
    filename = 'C:\Users\sebastian\ownCloud\Chip Design\General\pySon\parser_example\CPW_param2.s2p'
